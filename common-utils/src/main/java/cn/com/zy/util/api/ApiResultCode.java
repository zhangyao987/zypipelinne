...
public enum ApiResultCode {
    ...
    public static ApiResultCode deserializer(String key) {
        ApiResultCode deserializer = null;
        for (int i=0; i<ApiResultCode.values().length; i++) {
            if (ApiResultCode.values()[i].getKey().equals(key)) {
                deserializer = ApiResultCode.values()[i];
                break;
            }
        }
        return deserializer;
